...
test('buildPinoOptions - server config level fallback', t => {
  const pinoOptions = buildPinoOptions({}, { level: 'debug' }, 'test-application', 'worker-1', {}, import.meta.dirname)
...
test('buildPinoOptions - custom level', t => {
  const pinoOptions = buildPinoOptions({ level: 'info' }, {}, 'test-application', 'worker-1', {}, import.meta.dirname)
...
test('buildPinoOptions - full custom options', async t => {
  ...
  strictEqual(pinoOptions.name, 'test-application')
...
test('buildPinoOptions - invalid formatters.bindings value', t => {
  ...
  throws(
    () => {
      buildPinoOptions(loggerConfig, {}, 'test-application', 'worker-1', {}, import.meta.dirname)
    },
    err => {
...
test('loadFormatters - file exports correct formatters', t => {
  ...
  const bindingsResult = result.bindings({ application: 'my-application' })
  deepStrictEqual(bindingsResult, { application: 'my-application', app: 'test-app' })

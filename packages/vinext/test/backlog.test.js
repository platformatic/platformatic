...
for (const [env, options] of Object.entries(envs)) {
  test(`vinext application should properly use backlog option in ${env}`, async t => {
  ...
  test(`vinext application should properly use backlog option in ${env} when using custom commands`, async t => {
    const { runtime } = await prepareRuntime({
      t,
      ...
      async additionalSetup (root, config) {
        ...
        await updateConfigFile(resolve(root, 'services/frontend/platformatic.application.json'), frontendConfig => {
          frontendConfig.server ??= {}
          frontendConfig.server.backlog = 100
        })

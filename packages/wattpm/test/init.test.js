...
describe('init', async t => {
  test('should create a new application', async t => {
    const directory = await createTemporaryDirectory(t, 'init')
    await wattpm('init', directory)

    ok(isDirectory(resolve(directory, 'web')))

    deepStrictEqual(JSON.parse(await readFile(resolve(directory, 'wattpm.json'), 'utf-8')), {
      $schema: schema.$id,
  ...
  test('should fail if the destination is a file', async t => {
  ...
  test('should fail if the destination is not empty', async t => {

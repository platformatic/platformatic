...
app.get('/', async (req) => {
  const client = await generateMoviesClient({ url: '${runtimeUrl}' })
...
test('no such file (empty dir)', async t => {
  await rejects(execa('node', [join(import.meta.dirname, '..', 'index.js')]))
})
...
app.post('/', async (req) => {
  const client = await generateUncannyMoviesClient({ url: '${runtimeUrl}' })
  const res = await client.createMovie({ title: 'foo' })
  return res
})
...
`
  ...
  await fs.writeFile(join(dir, 'build', 'uncanny-movies', 'package.json'), JSON.stringify({ type: 'module' }))
...
export async function generateFullClient (opts) {
  return buildOpenAPIClient({
    type: 'openapi',
    name: 'full',
    path: join(import.meta.dirname, 'full.openapi.json'),
    url: opts.url,

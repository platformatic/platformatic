...
test('build basic client from url', async t => {
  ...
  const app = await create(join(import.meta.dirname, 'fixtures', 'sample', 'platformatic.db.json'))
...
test('test factory and client', async t => {
  ...
  // start 2 services
  const app = await create(join(import.meta.dirname, 'fixtures', 'sample', 'platformatic.db.json'))
  const app2 = await create(join(import.meta.dirname, 'fixtures', 'sample', 'platformatic.db.json'))
  t.after(async () => {
    await app.close()
    await app2.close()
  })
...
`
  ...
  await writeFile(join(dir, 'foobar', 'test.js'), testFile)

  // execute the command
  const output = await execa('node', [join(dir, 'foobar', 'test.js')])
...
test('do not add headers to fetch if a get request', async t => {
  ...
  const openAPIfile = join(import.meta.dirname, 'fixtures', 'empty-responses-openapi.json')
  await execa('node', [
...
test('call response.json only for json responses', async t => {
  ...
  {
    const openAPIfile = join(import.meta.dirname, 'fixtures', 'frontend-openapi.json')
    await execa('node', [
      join(import.meta.dirname, '..', 'index.js'),
      openAPIfile,
      '--name',
...
test('should match expected implementation with typescript', async t => {
  const dir = await moveToTmpdir(after)
  const openAPIfile = join(import.meta.dirname, 'fixtures', 'multiple-responses-openapi.json')
  await execa('node', [
    join(import.meta.dirname, '..', 'index.js'),
    openAPIfile,
    '--name',
...
test('serialize correctly array query parameters', async t => {
  const dir = await moveToTmpdir(after)
  {
    const openAPIfile = join(import.meta.dirname, 'fixtures', 'array-query-parameters-openapi.json')
    await execa('node', [
      join(import.meta.dirname, '..', 'index.js'),
      openAPIfile,
      '--name',
...
test('integration test for FormData handling', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'form-data')
  try {
  ...
  const app = await create(join(fixturesDir, 'platformatic.db.json'))
  t.after(async () => {
    await app.close()
  })

  await app.init()
  await app.getApplication().register(import('@fastify/multipart'))
...
`
  ...
  await writeFile(join(dir, 'formdata', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'formdata', 'test.js')])
  /* eslint-disable no-control-regex */
...
test('integration test for custom fetch parameters', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'fetch-params')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'fetch-params', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'fetch-params', 'test.js')])
  /* eslint-disable no-control-regex */
  const lines = output.stdout.replace(/\u001b\[.*?m/g, '').split('\n')
...
test('integration test for allOf and anyOf schema types', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'allof-anyof-schema')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'combined-types', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'combined-types', 'test.js')])
  /* eslint-disable no-control-regex */
  const lines = output.stdout.replace(/\u001b\[.*?m/g, '').split('\n')
...
test('integration test for optional headers', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'optional-headers')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'optheaders', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'optheaders', 'test.js')])
  /* eslint-disable no-control-regex */
  const lines = output.stdout.replace(/\u001b\[.*?m/g, '').split('\n')
...
test('integration test for optional query parameters', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'optional-query-params')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'optparams', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'optparams', 'test.js')])
  /* eslint-disable no-control-regex */
  const lines = output.stdout.replace(/\u001b\[.*?m/g, '').split('\n')
...
test('integration test for JSON and text response types', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'content-types')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'content-types', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'content-types', 'test.js')])
  /* eslint-disable no-control-regex */
  const lines = output.stdout.replace(/\u001b\[.*?m/g, '').split('\n')
...
test('integration test for simple 200 response with --full option', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'full-response-simple')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'fullresponse', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'fullresponse', 'test.js')])
...
test('integration test for 204 No Content responses', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'no-content-response')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'nocontent', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'nocontent', 'test.js')])
  /* eslint-disable no-control-regex */
...
test('integration test for query parameters', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'array-query-params')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
test('integration test for custom headers', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'custom-headers')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
test('integration test for DELETE without body', async t => {
  const fixturesDir = join(import.meta.dirname, 'fixtures', 'delete-no-body')
  try {
    await fs.unlink(join(fixturesDir, 'db.sqlite'))
  } catch {
    // noop
...
`
  ...
  await writeFile(join(dir, 'delete-api', 'test.js'), testFile)

  const output = await execa('node', [join(dir, 'delete-api', 'test.js')])
  /* eslint-disable no-control-regex */
  const lines = output.stdout.replace(/\u001b\[.*?m/g, '').split('\n')
...
test('add credentials: include in client implementation from file', async t => {
  const dir = await moveToTmpdir(after)
  {
    const openAPIfile = join(import.meta.dirname, 'fixtures', 'movies', 'openapi.json')
    await execa('node', [
      join(import.meta.dirname, '..', 'index.js'),
      openAPIfile,
      '--name',
...
import type * as Types from './client-types'`)
  ...
  ok(
    implementation.includes(`const _getHello = async (url: string, request: Types.GetHelloRequest): Promise<Types.GetHelloResponses> => {
  const headers: HeadersInit = {
    ...defaultHeaders
  }`)
  )
  ok(
    implementation.includes(`export const getHello: Client['getHello'] = async (request: Types.GetHelloRequest): Promise<Types.GetHelloResponses> => {
  return await _getHello(baseUrl, request)
}`)
  )
  ...
  ok(
    types.includes(`export interface Client {
  setBaseUrl(newUrl: string): void;
  setDefaultHeaders(headers: object): void;
  setDefaultFetchParams(fetchParams: RequestInit): void;
  /**
...
test('frontend client with full option', async t => {
  const dir = await moveToTmpdir(after)
  const openAPIfile = join(import.meta.dirname, 'fixtures', 'full-req-res', 'openapi.json')
  await execa('node', [
    join(import.meta.dirname, '..', 'index.js'),
    openAPIfile,
    '--name',
  ...
  ok(
    implementation.includes(`if (request.headers && request.headers['headerId'] !== undefined) {

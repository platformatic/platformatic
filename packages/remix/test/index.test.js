...
test('can detect and start a Remix application', async t => {
  await updateHMRVersion()
  const { url } = await createRuntime(t, 'remix/standalone/platformatic.runtime.json', packageRoot)

  const htmlContents = [/Hello from <!-- -->\d+/, 'window.__remixRouteModules']

  await verifyHTMLViaHTTP(url, '/', htmlContents)
  await verifyHMR(url, '/', 'vite-hmr', websocketHMRHandler)
})

test('can detect and start a Remix application when exposed in a composer with a prefix', async t => {
  await updateHMRVersion()
  const { runtime, url } = await createRuntime(t, 'remix/composer-with-prefix/platformatic.runtime.json', packageRoot)
...
test('can detect and start a Remix application when exposed in a composer without a prefix', async t => {
  await updateHMRVersion()
  const { runtime, url } = await createRuntime(
    t,
    'remix/composer-without-prefix/platformatic.runtime.json',
    packageRoot
  )
...
test('can detect and start a Remix application when exposed in a composer with a custom config and by autodetecting the prefix', async t => {
  await updateHMRVersion()
  const { runtime, url } = await createRuntime(
    t,
    'remix/composer-autodetect-prefix/platformatic.runtime.json',
    packageRoot
  )

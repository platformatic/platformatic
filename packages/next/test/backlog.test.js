...
for (const [env, options] of Object.entries(envs)) {
  test(`Next.js application should properly use backlog option in ${env}`, async t => {
  ...
  test(`Next.js application should properly use backlog option in ${env} when using custom commands`, async t => {
    const { runtime } = await prepareRuntime({
      t,

...
test('React Router version is checked in development', async t => {
  const { runtime, root } = await prepareRuntime(t, 'standalone', false, null, async () => {
    await swapVersion(t, import.meta.dirname, 'react-router', '../..')
  })

  await rejects(runtime.start())
  const logs = await getLogsFromFile(root)

  ok(logs.some(l => l.err?.message.includes('react-router version 1.0.0 is not supported')))
})

test('React Router version is not checked in production', async t => {

...
test('should execute a Nitro scheduled task through the Watt scheduler', async t => {
  ...
  const jobs = runtime.getScheduler()
  equal(jobs.length, 1)
  deepStrictEqual(jobs[0].tasks, ['smoke'])
  equal(jobs[0].source, 'application')

  const paused = await runtime.pauseSchedulerJob(jobs[0].name)

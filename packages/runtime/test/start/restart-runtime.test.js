...
test('do not restart if service is not started', async t => {
  ...
  assert.ok(logs.includes('The service \\"service-2\\" unexpectedly exited with code 1.'))

  assert.ok(logs.includes('Starting service \\"service-2\\" in 100ms. Attempt 1 of 5...'))
  assert.ok(logs.includes('Starting service \\"service-2\\" in 100ms. Attempt 2 of 5...'))
  assert.ok(logs.includes('Starting service \\"service-2\\" in 100ms. Attempt 3 of 5...'))
  assert.ok(logs.includes('Starting service \\"service-2\\" in 100ms. Attempt 4 of 5...'))
  assert.ok(logs.includes('Starting service \\"service-2\\" in 100ms. Attempt 5 of 5...'))

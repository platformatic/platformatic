...
test('extension metrics appear once in JSON and text output', async t => {
  ...
  const configFile = join(fixturesDir, 'extensions', 'metrics', 'watt.config.js')
...
test('multiple extension registries contribute distinct metrics', async t => {
  ...
  const configFile = join(fixturesDir, 'extensions', 'metrics-multi', 'watt.config.js')
...
test('extension metrics are not duplicated across multiple application workers', async t => {
  ...
  const configFile = join(fixturesDir, 'extensions', 'metrics-workers', 'watt.config.js')
...
test('metric-family collisions between extensions fail with a coded error', async t => {
  ...
  const configFile = join(fixturesDir, 'extensions', 'metrics-collision', 'watt.config.js')
  const app = await createRuntime(configFile)
  await app.start()
...
test('metric-family collisions with process metrics fail with a coded error', async t => {
  ...
  const configFile = join(fixturesDir, 'extensions', 'metrics-process-collision', 'watt.config.js')
  const app = await createRuntime(configFile)
  await app.start()
...
test('disabled metrics keep getMetrics unavailable and do not expose extension samples', async t => {
  ...
  const configFile = join(fixturesDir, 'extensions', 'metrics-disabled', 'watt.config.js')
  const app = await createRuntime(configFile)
  await app.start()

...
test(
  ...
  async t => {
    const configFile = join(fixturesDir, 'event-loop-delay', 'signals', 'platformatic.json')
...
test(
  ...
  { skip: isWindows && 'Skipping on Windows' },
  async t => {
    const configFile = join(fixturesDir, 'event-loop-delay', 'restart', 'platformatic.json')
    const app = await createRuntime(configFile)
...
test(
  ...
  { skip: isWindows && 'Skipping on Windows' },
  async t => {
    const configFile = join(fixturesDir, 'event-loop-delay', 'restart-p99', 'platformatic.json')
    const app = await createRuntime(configFile)
...
test(
  ...
  { skip: isWindows && 'Skipping on Windows' },
  async t => {
    const configFile = join(fixturesDir, 'event-loop-delay', 'disabled', 'platformatic.json')
    const app = await createRuntime(configFile)

...
async function wrapInRuntimeConfig (config, context) {
  let serviceId = 'main'
  try {
    const packageJson = join(config[kRoot], 'package.json')
...
async function transform (config, _, context) {
  ...
  if (config.autoload) {
    ...
    path = resolvePath(config[kRoot], path)
  ...
  for (let i = 0; i < services.length; ++i) {
    ...
    if (service.path && !isAbsolute(service.path) && !service.path.match(/^\{.*\}$/)) {
      service.path = resolvePath(config[kRoot], service.path)
    }

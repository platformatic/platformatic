...
const { features, ensureLoggableError, executeWithTimeout, deepmerge, kTimeout } = require('@platformatic/utils')
...
class Runtime extends EventEmitter {
  ...
  async #setupWorker (config, serviceConfig, workersCount, serviceId, index, enabled = true) {
    ...
    const maxOldGenerationSizeMb = Math.floor(
      (health.maxYoungGeneration > 0 ? health.maxHeapTotal - health.maxYoungGeneration : health.maxHeapTotal) /
        (1024 * 1024)
    )
    const maxYoungGenerationSizeMb = health.maxYoungGeneration
      ? Math.floor(health.maxYoungGeneration / (1024 * 1024))
      : undefined

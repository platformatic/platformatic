...
for (const warm of [false, true]) {
  test(`SIGINT can finish mesh HTTP and messaging I/O (warm channels: ${warm})`, async t => {
    const { runtime } = await prepareRuntime(t, 'signal-io', false)
    await startRuntime(t, runtime)
    if (warm) {
      strictEqual((await runtime.inject('frontend', { url: '/warm' })).body, 'ok')
    }
    const steps = []
    runtime.on('application:worker:event:signal:io:finished', () => steps.push('io:finished'))
    runtime.on('application:worker:stop:error', () => steps.push('error'))
    runtime.on('application:worker:stop:timeout', () => steps.push('timeout'))
    runtime.on('application:worker:exit:timeout', () => steps.push('exit:timeout'))
    await runtime.stopApplication('frontend')
    deepStrictEqual(steps, ['io:finished'])
    await runtime.close()
  })
}
...
test('shutdown waits for work scheduled by SIGINT before replying', async t => {
  ...
  runtime.on('application:worker:stopped', () => steps.push('stopped'))
  await runtime.close()
  deepStrictEqual(steps, ['signal:finished', 'stopped'])
})
...
test('startup failure preserves its error and completes async cleanup and SIGINT', async t => {
  ...
  deepStrictEqual(steps, ['callback', 'signal', 'signal:finished'])
  await runtime.close()
  deepStrictEqual(steps, ['callback', 'signal', 'signal:finished'])
})
...
test('should log stop errors without hanging runtime close', async t => {
  ...
  deepStrictEqual(stopErrorLog.err?.aggregateErrors.map(error => error.message), [
    'boom while closing',
    'boom while callback'
  ])
})

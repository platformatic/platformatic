...
test('Creates a Platformatic DB service with no migrations and no plugin', { timeout, skip: true }, async () => {
...
test('Creates a Platformatic DB service with migrations and plugin', { timeout, skip: true }, async () => {
  // The actions must match IN ORDER
  const actions = [{
    match: 'Which kind of project do you want to create?',
    do: [keys.ENTER]
...
test('Creates a Platformatic DB service with plugin using typescript, creating all the github actions', { timeout, skip: true }, async () => {
  // The actions must match IN ORDER
  const actions = [{
    match: 'Which kind of project do you want to create?',
    do: [keys.ENTER]

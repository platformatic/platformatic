...
| D9 | M2 | Non-entrypoint `server.port`: restore v3 (no listen) or breaking change + migrate strip | **resolved** — user decision: the new rule ships. A non-entrypoint whose own `server` block sets a port opens a real listener (a stable internal address is a feature). Numbered breaking change — v3 made a non-entrypoint's config `server.port` inert (`worker/controller.js:218`, `worker/main.js:274-283`); only `useHttp` listened. Migrate **strips `server.port` from non-entrypoint apps**, or every migrated monorepo carrying generator-written `"server": { "port": "{PORT}" }` blocks races for one port on first boot. Blackbox apps binding their own code's port are unaffected (`basic/lib/worker/listeners.js:24-29`) |
...
| D12 | M5 | `PORT`/3042 default vs v3's deference to the app's own `listen()` | **resolved** — user decision: **no terminal rung**. Entrypoint address resolution ends after the app's own `server` block (v3 parity — `buildListenOptions` is `{ port: serverConfig?.port \|\| 0 }`, `basic/lib/utils.js:21-29`; the same `\|\| 0` / `?? true` pattern holds in `basic/lib/capability.js:645` and `vite/lib/capability.js:229,242`). Blackbox apps keep their own `listen()` port; factory-style and framework entrypoints get an ephemeral port, which the runtime already pins for restarts (`runtime.js:3262-3271`). `3042` exists only in generator templates and stays there — scaffolded config expresses `Number(process.env.PORT ?? 3042)` explicitly, visibly, and editably. The `:594-597` parallel-standalone argument is rewritten: standalone processes no longer collide, they simply have no mesh. `reuseTcpPorts`/`portAssignment` are noted inert on an ephemeral entrypoint |
...
| D16 | M9 | Standalone builds: evaluate the ancestor root for `envfile`/withheld-keys, or rescope both claims | **resolved** — user decision: **standalone stays isolated**. "Standalone means standalone" remains absolute and cannot be broken by a malformed ancestor. Both claims are downgraded honestly instead: the build-determinism paragraph is rewritten as "identical **when no application declares `envfile` and no root/entry `env` block feeds config evaluation**", and the withheld-`env`-block-keys warning becomes **root-build-only**. The standalone-build hazard (`turbo run build` can emit different artifacts than a root build, unwarned) is documented in the migration guide beside the working-directory note at `:599-602`. Also fix the standalone warning at `:566-570`, which asserts an ancestor is a *root* config without evaluating it — classification requires execution |

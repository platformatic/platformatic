...
|---|---|---|---|
| E1 | B1 | Make `env` blocks visible to the root worker's resolution pass (root-inline entries), or have migrate refuse these shapes | **open** |
| E2 | B2 | Relax D17 for entries in the root config's own directory, or give migrate a promote rule | **open** |
| E3 | B3 | Scope the non-entrypoint `server.port` strip to carried-over ports only | **open** |
| E4 | B4 | Extend `reuseTcpPorts`/`portAssignment` to per-app blocks, or restrict non-entrypoint fixed ports to `workers === 1` | **open** |
| E5 | B5 | Decouple root-config location from the package.json execution-eligibility rules | **open** |
| E6 | M1, M2 | Compare post-transform, or expand the v3 side structurally; extend the vendored closure with the worker env layer | **open** |
| E7 | M3, M4 | One block/file ordering for both views; move `loadEnv` and provenance main-side | **open** |
| E8 | M5, M6, M7, M8 | Repair the env-model prose the D7 commit left stale or over-claimed | **open** |
| E9 | M9, M10 | Declare the `NODE_ENV`-under-build and entrypoint-auto-detection changes | **open** |
| E10 | M11, M12, M13 | Restore claim-check containment + realpath; define `--config` scope; define "root config" by classification | **open** |
| E11 | M14, M15, M16 | Specify ephemeral-entrypoint mechanics; `port: 0` + `portAssignment`; plumb entry-level `reuseTcpPorts` | **open** |
| E12 | M17 | Backfill remote-app paths in the root eval worker | **open** |
| E13 | M18, M19 | Fix the shared-block list and collision rule; scope the "never imports" claim | **open** |
| E14 | M20, M21, M22 | Capability-family-aware `useHttp`; rename before the gate; scope the `*_URL` carve-out | **open** |
| E15 | M23, M24, M25 | Envfile-promotion safety; re-install on rollback; per-app `package.json` rule | **open** |
| E16 | M26, M27, M28 | `management` object form; `ApplicationEntryOverrides`; programmatic env root | **open** |

Minors m1–m24: apply directly, no decision needed.

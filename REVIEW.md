...
| E6 | M1 | Equivalence-check comparand: post-transform, or pre-transform with the v3 side structurally expanded | **open** |
| E6b | M2 | Extend the vendored closure with the worker-boot env layer | **applied** — `worker/main.js:234-269` and `controller.js:94-100,143-148` added to the enumerated closure; the resolved view is built per application under that application's simulated worker environment |
| E7 | M3, M4 | One block/file ordering for both views; where `loadEnv` and provenance run | **open** |
...
| E15 | M23, M25 | Envfile-promotion safety; per-app `package.json` rule | **open** |

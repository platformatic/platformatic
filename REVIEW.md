...
|---|---|---|---|
| D1 | B1 | Standalone-boot port assignment (search/offset/refuse) and the rule for sibling-dependent capabilities (gateway) booted standalone | open |
| D2 | B2 | Legacy-detection scope (per-directory?), migrate's always-emit-per-app-files, and the validation loader's ignore-legacy flag | open |
| D3 | M1 | Cross-app ESM cache contamination in env windows: per-app eval workers, documented read-in-function contract, or env proxy | open |
| D4 | M2 | Does `mode` reach workers (workerData + mode-aware worker env file set), or do workers stay on `.env` only | open |
| D5 | M3 | Walk/loadEnv ordering fix, the standalone-boot env model, and invocation-dependent evaluation env | open |
| D6 | M4 | Version-stamp check defined against the worker's actual resolution order; strictness level (major+minor vs major) | open |
| D7 | M5 | `build`/`start` standalone semantics in CI/deploy: env parity for builds, guard rails for standalone `start` | open |

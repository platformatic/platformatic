...
When running in production mode, a custom Fastify server serves the built application in a worker thread. The Vite capability owns this listener and uses its capability-level `server` configuration.

...
* `--full-response` - Client will return full response object rather than just the body. Bear in mind that in some cases the full response will be forced (f.e. when generating the frontend the OpenAPI schema contains an empty response).

...
```json
{
  "$schema": "https://schemas.platformatic.dev/@platformatic/composer/2.66.1.json",
  "composer": {
    "services": [{
      "id": "node",
      "proxy": {
...
```json
{
  "$schema": "https://schemas.platformatic.dev/@platformatic/next/2.66.1.json",
  "application": {
    "basePath": "/next"
  }
}
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch('http://node.plt.local:3042/node');

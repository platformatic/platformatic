...
Continuous profiling also backs off automatically when a worker is overloaded: if the ELU rises above the worker's `health.maxELU` (0.99 by default), the current window still completes its full duration and is announced via the usual event — so your extension still ships the window that shows what saturated the worker — and then profiling pauses until the ELU recovers. The final profile stays available until profiling resumes, so it can also be retrieved at any point during the overload. Pass `maxELU` in the profiling options to change the cutoff, or `maxELU: false` to disable it.

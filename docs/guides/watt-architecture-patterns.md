...
- **Pyramid architecture**: one public application fans out to many applications.
- **Funnel architecture**: many identical CPU-bound instances feed one I/O-focused application.

Use these patterns to decide where to place public applications, CPU-heavy work, I/O-heavy work,
...
The Watt boundary contains the public application and all downstream applications. Clients sit
...
The public application can also run multiple workers, but keep its responsibilities focused on
routing, authentication, composition, and request coordination. Avoid putting all domain
logic in the public application, or it becomes the new monolith.

### Tradeoffs

Pyramid architecture gives you one clear public boundary and simple deployment, but the
public application can become a bottleneck if it does too much work. Cross-application request
...
This pattern fits systems where CPU saturation in the public application is the main bottleneck
...
The Watt boundary contains the public application, domain applications, CPU instances, and I/O
...
| --- | --- | --- |
| Public application | Routing, auth, composition, SSR | Request rate and latency |

...
export default defineComponent({
  data(): {
    ...
    newMovie: Awaited<ReturnType<typeof createMovie>> | undefined;
  } {
    return {
      movies: [],
      newMovie: undefined,
  ...
  methods: {
    ...
    async onUpdateMovie() {
      if (!this.newMovie || !this.newMovie.id) return

      const updatedMovie = await updateMovie({ id: this.newMovie.id, title: 'The Lord of the Rings' })
      this.newMovie = updatedMovie
    }
  },

...
(`runtime/lib/config.js:435-440` pre-`7f3ce97ef`, v3 semantics) — **but only once the two are known
...
(`wattpm-utils/lib/commands/external.js:540-668`). That matters in v4: `resolve`
...
`#setupApplication` (`runtime/lib/runtime.js:2511,2526`) because per-app config was
...
`type: 'unknown'` (`runtime/lib/config.js:236-238` pre-`7f3ce97ef`, whose comment gives the same
...
`id` for both entry shapes (`foundation/lib/schema.js:869-872`); v4 relaxes it for local
...
the applications whose path is missing (`wattpm-utils/lib/commands/external.js:566,585-596`),
...
- **`enabled` must not hide an entry from `resolve`.** It is the supported way to
  ...
  **Where that list comes from is the load-bearing part**, because `resolve` cannot
  ...
  (`external.js:566`) nothing is left holding the pre-filter list, and re-deriving it
  ...
  `application.gitBranch ?? parsedGitUrl.branch` (`external.js:421`), so a disabled
  ...
  **Main-side, the candidates ride on `kMetadata`.** `loadConfiguration` keeps
  ...
  filtering the returned application list as it does today (`external.js:585-596`).
  ...
  **`path` is the fourth field for the same reason `gitBranch` is the third**: it is
  ...
  (`external.js:585-615`), so a candidate is identified by its **effective
  ...
  **The map runs the other way as well: one destination, one producer.** Matching by
  ...
  any entry whose path already exists (`external.js:585-592`), and that is exactly the
...
dependency added to a cloned app's `package.json` (`external.js:347-382`, inside
...
that selected it (`external.js:190-201` pre-`6eca16405`). In v4 `import`
writes literal relative paths into the config (`external.js:217-262`) — an env-var indirection
...
4. an empty/other object is a root config — *classified* as one, which is not the
   ...
   `runtime/lib/schema.js:70-74`; the audit narrows it to the two v4 spellings plus
...
- **App-def nearest** → **that application boots standalone**: the definition is
  ...
  (`runtime/lib/config.js:424` pre-`7f3ce97ef`, `mapping.id ?? entry.name`). Stripping the scope is not cosmetic: the id becomes a
...
(`runtime/lib/runtime.js:1046,1082`), spawned with the environment the loader
resolved, both `env` blocks on its ladder (`runtime/lib/runtime.js:2653`) — and a
...
production flag (`wattpm/lib/commands/build.js:43` pre-`b1e03a5b9` → `runtime.js:306`), so
`worker/controller.js:128-129` never fired and builds ran with `NODE_ENV` unset.
...
- **A fixed port with `workers > 1` requires `SO_REUSEPORT`, and there is no
  ...
  **entry-level one now reaches the decision** — the runtime passes the whole
  ...
  (`runtime/lib/runtime.js:2559-2575`, `edf83931d`, closing
...
- **`application.entrypointPort` is removed** for the same reason as custom
  ...
  (`runtime.js:5073-5089` → `:5043-5064`), so two applications on genuinely distinct
...
- **Two applications cannot share a port.** When a worker reports a URL, the
  ...
  `runtime/lib/runtime.js:5087-5089`, ownership scan at `runtime/lib/runtime.js:5043-5064`). Workers of
  the *same* application are exempt by construction (`runtime/lib/runtime.js:5049`), which is what makes
  `SO_REUSEPORT` legal. An OS-level `EADDRINUSE` carrying a port is upgraded to
  the same error (`runtime/lib/runtime.js:3474-3484`), and `EADDRINUSE` / `EACCES` / `EADDRNOTAVAIL`
  are excluded from restart-on-error (`runtime/lib/runtime.js:3512-3516`) — a port problem fails fast
  ...
  **Host overlap is a table, deliberately a small one.** Two ranges conflict only if
  ...
  must be equal, case-insensitively (`runtime/lib/runtime.js:5066-5071`). Anything
...
- **The runtime reports a map of URLs, not one URL.** `getUrls(applicationId?)`
  ...
  (`runtime/lib/runtime.js:1684-1698`); `start()` returns it (`runtime/lib/runtime.js:518`) after
  logging one line per **application** rather than one per listening worker
  (`#showUrls`, `runtime/lib/runtime.js:2478-2498`) — an application with several workers on distinct
  ports lists them together rather than N times over. `getRuntimeMetadata()` carries
  `urls` (`runtime/lib/runtime.js:1725`), `getApplicationDetails()` carries `urls` plus a first-element
  `url` convenience (`runtime/lib/runtime.js:2237-2239`), and worker records carry their own `url`
  (`runtime/lib/runtime.js:2356`). `wattpm ps` dropped its URL column and `wattpm applications` its
...
schema, `skipTracingHooks` and `modulesToLoad`; it gains `servesWithoutPort`, read
...
changes is the report. `#showUrls` (`runtime/lib/runtime.js:2474-2494`) currently
...
emission on `status === 'started'` (`runtime/lib/runtime.js:2237`), so overloading it
...
warning (`wattpm-utils/lib/commands/external.js:608-616`), while a directory that
already exists is used as-is whatever its location. The check is a path-boundary
test (`isPathInsideDirectory`, `wattpm-utils/lib/commands/external.js:268-275`), so
...
ancestor inside the root still redirects the write outside it. The format inherits
the rule, not any particular implementation of it.
...
1. **The root worker** is constructed by the main process with an explicit
   ...
   **unwraps** the default export — and unwrapping is *only* the function call: if
   ...
   4. **Expand `autoload`** into the application list, reading `autoload.path`,
      ...
      which is what v3 did (`foundation/lib/configuration.js:387-407`, validate then
...
2. **One worker per per-app file**, spawned in parallel once the root result is
   ...
   **config-evaluation** environment by that view of the ladder — real environment,
   ...
   preparation needs besides the schema: `skipTracingHooks` (which decides
   whether the worker gets the OpenTelemetry `--import` hook — `runtime.js:2623`,
...
either runs. v3 did both here (`runtime/lib/config.js:409-443` pre-`7f3ce97ef` then `:445-449` pre-`7f3ce97ef`), and
...
(v3 required it for the same reason, `foundation/lib/configuration.js:357,367-368`).
...
(`worker/controller.js:128-129`, applied after all seeding), a truthiness test — so
...
outright (`foundation/lib/configuration.js:349-357` pre-`7a541feae`), and it is what makes the flag an
...
`.plt.local` URL (`runtime/lib/worker/controller.js:34-40`); in the *root* config,
...
- **In application code**: the runtime injects `PLT_<ID>_URL` environment variables
  ...
  **`PLT_ROOT` carries two different values in v3, and removing it breaks both.** The
  ...
  (`foundation/lib/configuration.js:522` pre-`7a541feae`) — so inside a configuration `{PLT_ROOT}`
...
- **AJV stays authoritative**, but the v4 schemas are **audited, not just pruned**.
  ...
  (`runtime/lib/config.js:330-346` pre-`7f3ce97ef`). `enabled` is resolved in the root eval worker against
...
- **`getApplicationConfig()` is a different API with a different view, and it
  ...
  (`runtime/lib/runtime.js:2248-2252` through `runtime/lib/worker/itc.js:324-328`), so
  ...
  (`runtime/lib/runtime.js:3675`), which is the same reason. It is already a copy —
...
- **The programmatic payloads are a versioned public DTO, and they change shape.**
  ...
  (`runtime/lib/runtime.js:2195-2211`) — received a *file path* in v3; in
  ...
  **both** `configPath` (the per-app file path, or absent for inline definitions)
  ...
  over ITC by `getApplicationInfo` (`runtime/lib/runtime.js:2199`). The two are
...
- **`wattpm applications:add` / `applications:remove`** keep their live half —
  ...
  project directory as `projectDir` (`runtime/lib/runtime.js:1713`) and is extended
  ...
  `autoload.exclude` (`wattpm/lib/commands/applications.js:167-168`), and that is
...
`while (!configurationFile)` (`foundation/lib/configuration.js:181-210`) — so a
...
- foundation's machinery: the parsers for all formats (JSON, JSON5, YAML, TOML),
  ...
  **v3 → v4 module rename table** (`@platformatic/composer` →
  ...
  run, `foundation/lib/configuration.js:140-165` called at `:373`, ahead of `upgrade()`
  at `:375`, so composer-era apps
...
- the **lexical view** — the parsed file with placeholder tokens
  ...
  `/(?:\{{1,2})([a-z0-9_]+)(?:\}{1,2})/i`, so `{PORT}`, `{{DATABASE_URL}}` and
  `{plt_x}` are all placeholders. Migrate carries that grammar itself
  (`wattpm-utils/lib/commands/migrate.js:85-87`) now that foundation's copy is gone with the
  interpolator — and it has to match it exactly: writing it narrower would silently leave
  un-converted tokens as literal text in the emitted configuration, where v3 had substituted
  them. *Which applications exist* is
  ...
  **module list** — what the
  ...
  (`foundation/lib/configuration.js:141-142`, `foundation/lib/module.js:129-140`);
...
(`runtime/lib/config.js:409-443` pre-`7f3ce97ef`, `:445-449` pre-`7f3ce97ef`) while v4 does both in
...
1. Emit the v4 files: for a v3 **single-app** project, one root file — the bare
   ...
   **Two autoloaded directories that resolve to the same id are a boot error naming
   ...
   carry the same name, and v3's shallow merge (`runtime/lib/config.js:435-440` pre-`7f3ce97ef`)
   ...
   *explicit* one, not for two autoloaded directories colliding.
   ...
   (`runtime/lib/config.js:424` pre-`7f3ce97ef`, `mapping.id ?? entry.name`); v4 prefers the
   ...
   **One final-id table serves everything downstream.** Explicit entries and autoloaded
   ...
   `runtime/lib/config.js:236-238` pre-`7f3ce97ef`), and appears on the migrate-the-other-repository
   ...
   **The directories `resolve` cloned into are excluded from every step** — the
   ...
   (`external.js:605`), so those directories hold *other repositories'* v3
   ...
   **An enum position gets a guard that returns the enum**, not a `string`. Migrate
   ...
   `foundation/lib/configuration.js:551` pre-`7a541feae` with
   ...
   **The count is taken after replaying v3's `enabled` splice, not over every
   ...
   (`config.js:330-346` pre-`7f3ce97ef`) over the lexical values and resolves the entrypoint
   **twice, once for `production` and once for `development`** — the only two
   values v3 derives (`:350` pre-`7f3ce97ef`).
   ...
   **undecidable** `enabled`: a string that was a `{PLT_X}` placeholder is unknown at
   ...
   (`foundation/lib/configuration.js:349-357` pre-`7a541feae`), so any automatic conversion either
   ...
   **resolved, not converted** (`wattpm-utils/lib/commands/migrate.js:534`): migrate
   ...
   **The two config positions are the easiest to leave off a list like this**, because
   ...
   application's own `path` (`runtime/lib/config.js:229-230` pre-`7f3ce97ef`), and accepted a filename
   in `autoload.mappings[].config`, joined to the discovered entry directory
   (`runtime/lib/config.js:428-431` pre-`7f3ce97ef`) — so `config: '{APP_CONFIG}'` is a legal v3
   ...
   (`foundation/lib/configuration.js:522` pre-`7a541feae`), so it was defined in every config parse
   ...
   (`wattpm-utils/lib/commands/migrate.js:477`) that decides it rather than a rule of
   ...
   *structure* rather than to preserve a value, and the report says which link in the
   ...
   has not been resolved (`wattpm-utils/lib/commands/external.js:342-370` pre-`6eca16405`, the
...
2. **Audit and install v4 dependencies before validating anything.** The emitted
   ...
   **The bound is every path migrate touches, not only application directories.** An
   ...
   the application's own path (`runtime/lib/config.js:229-230` pre-`7f3ce97ef`), so an application
   ...
   **A legacy configuration migrate is not converting** stops the run for the
   ...
   sources share a directory (`wattpm-utils/lib/commands/migrate.js:1776`). An earlier
   ...
   **Positions are grouped by the environment that supplies them, not by variable
   ...
   (`foundation/lib/configuration.js:361-371` pre-`7a541feae`) — so `web/a/.env` and `web/b/.env` can
   ...
   paths already use (`wattpm-utils/lib/commands/migrate.js:477`) — and intersects
...
3. **Validate the emitted files by explicit path**: migrate loads the generated
   ...
   (`runtime/lib/runtime.js:2523`), so a `wattpm resolve`-d clone *does* exist on
...
6. `verticalScaler`: removed from the v4 schema. `metrics.healthChecksTimeouts` is
   **kept** — it is not a top-level key and is not dead: `#getHealthChecksTimeout`
   reads it (`runtime/lib/runtime.js:4794-4801`, falling back to `healthChecksTimeout`
...
20. Injected variables: `PLT_DEV`, `PLT_ENVIRONMENT` and `PLT_ROOT` are
    **removed from worker environments** (the migrate source scan flags reads).
    ...
    matches v3 (`worker/controller.js:128-129`); under `build` it is **new** — v3's
...
22. **Listener mechanics and reporting.** Multi-worker on a fixed port requires
    ...
    (`runtime/lib/runtime.js:2559-2575`) — a start-time decision, not a
    configuration-format one. The
    **per-application `reuseTcpPorts` now reaches the `SO_REUSEPORT` decision**
    (`basic/lib/capability.js:105-110`, fed by `worker/controller.js:85`), where
...
24. **`wattpm import` writes different output.** It emits the v4 per-app config
    form instead of a `watt.json` `$schema` stub (`external.js:190-201` pre-`6eca16405`, whose
    selecting branch died with the legacy roots), and writes **literal relative paths**
    into the root config rather than the `{PLT_APPLICATION_<ID>_PATH}` placeholder
    plus `.env` line it wrote in v3 (`external.js:342-370` pre-`6eca16405`) — an env-var indirection
...
1. **foundation — a fresh loader, not a refactor.** The v4 loader is written new for
   ...
   **The interpolation half is gone**: `loadEnv`, `replaceEnv`, `envVariablePattern`,
   the `strictEnv` reporting and the `kEnvFileFallbackKeys` plumbing that carried its
   result into every worker. Nothing imported them any more — the last caller was the
   gateway's request-time `replaceEnv` over an origin the loader had already produced —
   and what remains of `loadConfiguration` parses a document, runs `upgrade`, validates,
   and calls `transform`. It resolves no environment and substitutes nothing, which is
   the whole of the difference between reading a configuration and evaluating one. The
   ~220 lines and their 43 tests went with them rather than moving: migrate translates a
   placeholder into an expression instead of substituting it, so its legacy reader never
   wanted this half.
2. **Schema audit** (foundation + all capabilities): classify the union sites, delete
   ...
   **Two consequences of the projection not existing turned up while converting the
   reference documentation to this format, and both are fixed.** The first is what the
   branch bought a wrong value: `server.port` was `integer | string`, so
...
4. **runtime**: delete `wrapInRuntimeConfig` and alias merging; entry `config`
   ...
   the runtime transform** (`runtime/lib/config.js:409-443` pre-`7f3ce97ef`, `:445-449` pre-`7f3ce97ef`) for the root
...
7. **wattpm-utils**: `wattpm import` via magicast with snippet fallback;
   ...
   (`external.js:585-596`) — the pre-filter list does not survive the eval worker, so
...
**Every code block in this document is checked in CI, or explicitly marked as
   ...
   **The gate covers the documentation pages that teach this format, not only this
   ...
   than resolves, the option types those blocks name that the generated types do not
   carry — declaring them into existence would let the gate read as though the
   document were fully checked. Most of those names are real now: the schemas carry
   titles, so the generator hoists each shared block into an interface the blocks
   import instead of inlining an anonymous object eighteen times. The ones still
   reported all live inside the runtime's **application entry**, which the pinned
   generator collapses to `{ [k: string]: unknown }` because the schema lists that
   entry three times — as `applications` and as the two v3 aliases. Two copies
   generate correctly and three do not; the generator's next major handles all three,
   and so does a `$ref` for the aliases under it, but that release is hours old and
   this repository's minimum-release-age policy refuses it. Every capability schema
   lists the entry once and generates those four names correctly today. The inventory is deliberately not quoted here, because
...
// generated from the audited schema and re-exported by `wattpm`
import type {
...
// generated from the audited schema and re-exported by the capability
import type {
  AppLoggerOptions,
  AppServerOptions,
...
truthiness (`runtime/lib/runtime.js:398`), so `''` is off and **any** non-empty
